...
export function usableDiscountCode(productId: string) {
  return {
    isActive: true,
    AND: [
      {
...
export function getDiscountCodeAmount(
  ...
  switch (discountCode.discountType) {
    ...
    default:
      throw new Error(
        `Invalid discountType ${discountCode.discountType satisfies never}`
      );
  }
}
